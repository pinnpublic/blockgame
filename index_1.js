...
function generatePattern() {
    ...
    const totalPatternWidth = patternWidth * PATTERN_SCALE * (blockWidth + BLOCK_GAP);
    const leftOffset = (LOGICAL_WIDTH - totalPatternWidth) / 2;
    const topOffset = 60;
            ...
            if (pattern[r][c] === '1') {
                for (let scaleY = 0; scaleY < PATTERN_SCALE; scaleY++) {
                    for (let scaleX = 0; scaleX < PATTERN_SCALE; scaleX++) {
                        const finalC = c * PATTERN_SCALE + scaleX;
                        const finalR = r * PATTERN_SCALE + scaleY;
                        const hue = baseHue + (finalC * 2) - (finalR * 2);
                        blocks.push({
                            x: leftOffset + finalC * (blockWidth + BLOCK_GAP),
                            y: topOffset + finalR * (BLOCK_HEIGHT + BLOCK_GAP),
                            width: blockWidth, height: BLOCK_HEIGHT,
                            health: Math.floor(Math.random() * 100) + 1,
                            specialType: 'NONE', color: `hsl(${hue}, 80%, 60%)`
                        });
                    }
                }
            }
    ...
    // ✨ [핵심 수정] 특수 블록의 체력(카운트)을 낮춥니다.
    pickedIndices.forEach(index => {
        const block = blocks[index];
        if (Math.random() < 0.5) {
            block.specialType = 'EXPLOSION'; // 빨간 블록
            block.color = `hsl(0, 80%, 60%)`;
            // 기존: 1~100 사이의 높은 체력을 그대로 사용
            // 변경: 1~10 사이의 낮은 체력으로 재설정
            block.health = Math.floor(Math.random() * 10) + 1;
        } else {
            block.specialType = 'MULTIPLIER'; // 파란 블록
            block.color = `hsl(210, 85%, 60%)`;
            // 기존: 1~10 사이의 체력
            // 변경: 1~3 사이의 더 낮은 체력으로 재설정
...
function draw() {
    ...
    // ✨ [추가] 다른 게임 요소들 위에 주사위 애니메이션을 그립니다.
    drawDiceAnimation();
...
function drawAimLine() {
    ...
    for (let i = 1; i <= dotCount; i++) {
        ...
        let color = `rgba(255, 255, 255, ${0.7 * (1 - i / dotCount)})`;
        if (selectedBallType === 'BOMB') {
            color = `rgba(225, 29, 72, ${0.9 * (1 - i / dotCount)})`;
        } else if (selectedBallType === 'MAGMA') {
            color = `rgba(249, 115, 22, ${0.9 * (1 - i / dotCount)})`;
        } else if (selectedBallType === 'DICE') {
            color = `rgba(59, 130, 246, ${0.9 * (1 - i / dotCount)})`; // ✨ [추가]
        }
